...
Description:
  Format tracked C/C++ source and header files using clang-format.
...
if [[ "${MODE}" == "check" ]]; then
  list_cpp_files | xargs -0 -r "${CLANG_FORMAT_BIN}" --dry-run --Werror --style=file
  echo "clang-format check passed for tracked C/C++ files."
else
  list_cpp_files | xargs -0 -r "${CLANG_FORMAT_BIN}" -i --style=file
  echo "Formatted tracked C/C++ files."
fi
