...
parameter_string(const hardware_interface::HardwareInfo &info,
...
std::optional<double>
parameter_double(const hardware_interface::HardwareInfo &info,
                 std::string_view name) {
...
parameter_size(const hardware_interface::HardwareInfo &info,
               std::string_view name) {
  const auto value = parameter_string(info, name);
  if (!value.has_value()) {
    return std::nullopt;
...
from_nanoseconds(std::int64_t nanoseconds) {
  if (nanoseconds <= 0) {
    return {};
  }
...
public:
  void store(const RefereeSnapshot &snapshot) noexcept {
    ...
    stage_remain_time_.store(snapshot.stage_remain_time,
                             std::memory_order_relaxed);
    self_hp_.store(snapshot.self_hp, std::memory_order_relaxed);
    max_hp_.store(snapshot.max_hp, std::memory_order_relaxed);
    chassis_power_limit_.store(snapshot.chassis_power_limit,
                               std::memory_order_relaxed);
    chassis_power_.store(snapshot.chassis_power, std::memory_order_relaxed);
    chassis_power_buffer_.store(snapshot.chassis_power_buffer,
                                std::memory_order_relaxed);
    shooter_heat_17mm_1_.store(snapshot.shooter_heat_17mm_1,
                               std::memory_order_relaxed);
    shooter_heat_17mm_2_.store(snapshot.shooter_heat_17mm_2,
                               std::memory_order_relaxed);
    shooter_heat_42mm_.store(snapshot.shooter_heat_42mm,
                             std::memory_order_relaxed);
    heat_limit_17mm_.store(snapshot.heat_limit_17mm, std::memory_order_relaxed);
    cooling_rate_17mm_.store(snapshot.cooling_rate_17mm,
                             std::memory_order_relaxed);
    projectile_allowance_17mm_.store(snapshot.projectile_allowance_17mm,
                                     std::memory_order_relaxed);
    projectile_allowance_42mm_.store(snapshot.projectile_allowance_42mm,
                                     std::memory_order_relaxed);
    remaining_gold_coin_.store(snapshot.remaining_gold_coin,
                               std::memory_order_relaxed);
    for (std::size_t index = 0; index < game_robot_hp_.size(); ++index) {
      game_robot_hp_[index].store(snapshot.game_robot_hp[index],
                                  std::memory_order_relaxed);
    }
    for (std::size_t index = 0; index < radar_mark_progress_.size(); ++index) {
      radar_mark_progress_[index].store(snapshot.radar_mark_progress[index],
                                        std::memory_order_relaxed);
    }
    radar_double_effect_chance_.store(snapshot.radar_double_effect_chance,
                                      std::memory_order_relaxed);
    radar_double_effect_active_.store(snapshot.radar_double_effect_active,
                                      std::memory_order_relaxed);
    dart_remaining_time_.store(snapshot.dart_remaining_time,
                               std::memory_order_relaxed);
    dart_latest_hit_target_.store(snapshot.dart_latest_hit_target,
                                  std::memory_order_relaxed);
    dart_hit_count_.store(snapshot.dart_hit_count, std::memory_order_relaxed);
    dart_selected_target_.store(snapshot.dart_selected_target,
                                std::memory_order_relaxed);
    last_update_ns_.store(to_nanoseconds(snapshot.last_update),
  ...
  RefereeSnapshot load() const noexcept {
    return RefereeSnapshot{
        ...
        .chassis_power_limit =
            chassis_power_limit_.load(std::memory_order_relaxed),
        .chassis_power = chassis_power_.load(std::memory_order_relaxed),
        .chassis_power_buffer =
            chassis_power_buffer_.load(std::memory_order_relaxed),
        .shooter_heat_17mm_1 =
            shooter_heat_17mm_1_.load(std::memory_order_relaxed),
        .shooter_heat_17mm_2 =
            shooter_heat_17mm_2_.load(std::memory_order_relaxed),
        ...
        .projectile_allowance_17mm =
            projectile_allowance_17mm_.load(std::memory_order_relaxed),
        .projectile_allowance_42mm =
            projectile_allowance_42mm_.load(std::memory_order_relaxed),
        .remaining_gold_coin =
            remaining_gold_coin_.load(std::memory_order_relaxed),
        .game_robot_hp = load_array(game_robot_hp_),
        .radar_mark_progress = load_array(radar_mark_progress_),
        .radar_double_effect_chance =
            radar_double_effect_chance_.load(std::memory_order_relaxed),
        .radar_double_effect_active =
            radar_double_effect_active_.load(std::memory_order_relaxed),
        .dart_remaining_time =
            dart_remaining_time_.load(std::memory_order_relaxed),
        .dart_latest_hit_target =
            dart_latest_hit_target_.load(std::memory_order_relaxed),
        .dart_hit_count = dart_hit_count_.load(std::memory_order_relaxed),
        .dart_selected_target =
            dart_selected_target_.load(std::memory_order_relaxed),
        .last_update =
            from_nanoseconds(last_update_ns_.load(std::memory_order_acquire)),
    };
  }

private:
  template <std::size_t Size>
  static std::array<double, Size>
  load_array(const std::array<std::atomic<double>, Size> &values) noexcept {
    auto result = std::array<double, Size>{};
    for (std::size_t index = 0; index < Size; ++index) {
      result[index] = values[index].load(std::memory_order_relaxed);
    }
    return result;
  }
  ...
  std::array<std::atomic<double>, 16> game_robot_hp_{};
  std::array<std::atomic<double>, 6> radar_mark_progress_{};
  std::atomic<double> radar_double_effect_chance_{0.0};
  std::atomic<double> radar_double_effect_active_{0.0};
  std::atomic<double> dart_remaining_time_{0.0};
  std::atomic<double> dart_latest_hit_target_{0.0};
  std::atomic<double> dart_hit_count_{0.0};
  std::atomic<double> dart_selected_target_{0.0};
...
public:
  ...
  on_init(const hardware_interface::HardwareComponentInterfaceParams &params)
    ...
    baudrate_ =
        static_cast<int>(parameter_double(info, "baudrate").value_or(115200.0));
    ...
    transfer_path_ =
        parameter_string(info, "transfer_path")
    ...
    tx_queue_ = std::make_unique<rmgo_utility::utility::RingBuffer<TxFrame>>(
        std::max<std::size_t>(
            parameter_size(info, "tx_queue_capacity").value_or(64), 1));
  ...
  export_command_interfaces() override {
    return export_scalar_command_interfaces(command_interfaces_,
                                            command_values_);
  }
  ...
  read(const rclcpp::Time & /*time*/,
    ...
    state_values_[to_index(StateIndex::game_stage_remain_time)] =
        snapshot.stage_remain_time;
    state_values_[to_index(StateIndex::hp)] = snapshot.self_hp;
    state_values_[to_index(StateIndex::max_hp)] = snapshot.max_hp;
    state_values_[to_index(StateIndex::shooter_cooling)] =
        snapshot.cooling_rate_17mm;
    state_values_[to_index(StateIndex::shooter_heat_limit)] =
        snapshot.heat_limit_17mm;
    state_values_[to_index(StateIndex::shooter_bullet_allowance)] =
        snapshot.projectile_allowance_17mm;
    state_values_[to_index(StateIndex::shooter_1_heat)] =
        snapshot.shooter_heat_17mm_1;
    state_values_[to_index(StateIndex::shooter_2_heat)] =
        snapshot.shooter_heat_17mm_2;
    state_values_[to_index(StateIndex::chassis_power_limit)] =
        snapshot.chassis_power_limit;
    state_values_[to_index(StateIndex::chassis_power)] = snapshot.chassis_power;
    state_values_[to_index(StateIndex::chassis_buffer_energy)] =
        snapshot.chassis_power_buffer;
    state_values_[to_index(StateIndex::radar_mark_hero)] =
        snapshot.radar_mark_progress[0];
    state_values_[to_index(StateIndex::radar_mark_engineer)] =
        snapshot.radar_mark_progress[1];
    state_values_[to_index(StateIndex::radar_mark_infantry_3)] =
        snapshot.radar_mark_progress[2];
    state_values_[to_index(StateIndex::radar_mark_infantry_4)] =
        snapshot.radar_mark_progress[3];
    state_values_[to_index(StateIndex::radar_mark_infantry_5)] =
        snapshot.radar_mark_progress[4];
    state_values_[to_index(StateIndex::radar_mark_sentry)] =
        snapshot.radar_mark_progress[5];
    state_values_[to_index(StateIndex::radar_double_effect_chance)] =
        snapshot.radar_double_effect_chance;
    state_values_[to_index(StateIndex::radar_double_effect_active)] =
        snapshot.radar_double_effect_active;
    state_values_[to_index(StateIndex::dart_remaining_time)] =
        snapshot.dart_remaining_time;
    state_values_[to_index(StateIndex::dart_latest_hit_target)] =
        snapshot.dart_latest_hit_target;
    state_values_[to_index(StateIndex::dart_hit_count)] =
        snapshot.dart_hit_count;
    state_values_[to_index(StateIndex::dart_selected_target)] =
...
private:
  ...
  static constexpr std::size_t to_index(CommandIndex index) {
    return std::to_underlying(index);
  }
  ...
  public:
    explicit Endpoint(RefereeSystemInterface &owner) : owner_(owner) {}
  ...
  RefereeTransferResult
  enqueue_clear_ui_operation(std::uint8_t operation_type,
    ...
    const std::uint16_t client_id =
        rmgo_core::referee::client_id_from_robot_id(robot_id);
    ...
    return enqueue_tx(static_cast<std::uint16_t>(
                          rmgo_core::referee::CommandId::student_interactive),
  ...
  std::optional<RefereeTransferResult> enqueue_selected_clear_layer() noexcept {
    const auto layer =
        command_layer(command_values_[to_index(CommandIndex::clear_layer)]);
    if (layer.has_value()) {
  ...
  bool open_serial() {
    ...
    if (serial_fd_ == invalid_fd) {
      RCLCPP_ERROR(logger_, "Failed to open referee serial device '%s': %s",
  ...
  bool configure_serial() {
    ...
    if (::tcgetattr(serial_fd_, &options) != 0) {
      RCLCPP_ERROR(logger_, "Failed to read referee serial options: %s",
                   std::strerror(errno));
      return false;
    }
    ...
    if (::cfsetispeed(&options, speed) != 0 ||
      ...
      RCLCPP_ERROR(logger_, "Failed to set referee serial baudrate: %s",
                   std::strerror(errno));
      return false;
    }
    if (::tcsetattr(serial_fd_, TCSANOW, &options) != 0) {
      RCLCPP_ERROR(logger_, "Failed to apply referee serial options: %s",
                   std::strerror(errno));
      return false;
    }
  ...
  void start_transport() {
    if (serial_fd_ == invalid_fd || rx_thread_.joinable() ||
        tx_thread_.joinable()) {
      return;
    }
    ...
    rx_thread_ = std::jthread{
        [this, fd](std::stop_token stop_token) { rx_loop(stop_token, fd); }};
    tx_thread_ = std::jthread{
        [this, fd](std::stop_token stop_token) { tx_loop(stop_token, fd); }};
  }
  ...
  void rx_loop(std::stop_token stop_token, int fd) {
    auto rx_buffer =
        std::vector<std::byte>(std::max<std::size_t>(rx_buffer_size_, 1));
    while (!stop_token.stop_requested()) {
      ...
      if (count < 0) {
        ...
        RCLCPP_WARN(logger_, "Referee serial read failed: %s",
                    std::strerror(errno));
      ...
      for (ssize_t index = 0; index < count; ++index) {
        if (const auto frame =
                parser_.push(rx_buffer[static_cast<std::size_t>(index)]);
            frame.has_value()) {
          update_robot_id(*frame);
          if (rmgo_core::referee::apply_frame_to_snapshot(*frame,
                                                          latest_snapshot_)) {
  ...
  void tx_loop(std::stop_token stop_token, int fd) {
    ...
    auto packed =
        std::array<std::byte, rmgo_core::referee::max_referee_frame_size>{};
    while (!stop_token.stop_requested()) {
      if (tx_queue_ == nullptr ||
          !tx_queue_->pop_front(
      ...
      const auto payload = std::span<const std::byte>{tx_frame.payload}.first(
      ...
      if (!write_all(stop_token, fd,
                     std::span<const std::byte>{packed}.first(*packed_size)) &&
  ...
  bool write_all(std::stop_token stop_token, int fd,
    ...
    while (!stop_token.stop_requested() && written < bytes.size()) {
      ...
      if (count < 0) {
        ...
        RCLCPP_WARN(logger_, "Referee serial write failed: %s",
                    std::strerror(errno));
        return false;
      }
  ...
  void update_robot_id(const rmgo_core::referee::RefereeFrame &frame) noexcept {
    if (frame.command_id != static_cast<std::uint16_t>(
                                rmgo_core::referee::CommandId::robot_status) ||
        frame.payload.empty()) {
      return;
    }
    robot_id_.store(read_u8(std::span<const std::byte>{frame.payload}, 0),
                    std::memory_order_release);
  }

  rclcpp::Logger logger_{
      rclcpp::get_logger("rmgo_core.referee_system_interface")};
  ...
  std::array<double, std::to_underlying(CommandIndex::count)>
      previous_command_values_{};
...
PLUGINLIB_EXPORT_CLASS(rmgo_core::interface::RefereeSystemInterface,
