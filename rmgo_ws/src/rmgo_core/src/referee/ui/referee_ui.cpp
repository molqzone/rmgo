...
std::pair<InteractiveDataCommandId, std::size_t>
draw_command_for_count(std::size_t count) {
  if (count <= 1) {
...
void append_no_operation_description(std::span<std::byte> payload,
                                     std::size_t &written) {
...
UiScheduler::update(std::string_view transfer_path,
...
std::optional<RefereeTransferResult>
UiScheduler::update(RefereeTransferEndpoint &endpoint,
                    std::chrono::steady_clock::time_point now) {
  ...
  auto payload = std::array<std::byte, interaction_header_size +
                                           7 * graphic_description_size>{};
  std::size_t payload_size = interaction_header_size;

  std::stable_sort(run_queue_.begin(), run_queue_.end(),
  ...
  while (!run_queue_.empty() && selected_count < selected.size()) {
    ...
    if (!shape->write_update(payload, payload_size,
  ...
  if (!pack_interactive_payload(payload, *header, std::span<const std::byte>{})
           .has_value()) {
    requeue_selected(std::span<Shape *const>{selected}.first(selected_count));
  ...
  next_interaction_send_ =
      now + std::max(interaction_period_,
...
UiScheduler::clear_all(RefereeTransferEndpoint &endpoint) const {
  auto payload = std::array<std::byte, interaction_header_size + 2>{};
  const auto header = make_client_interactive_header(
      endpoint, InteractiveDataCommandId::client_delete);
  if (!header.has_value()) {
    return RefereeTransferResult::Inactive;
  }
  if (!pack_interactive_payload(payload, *header, std::span<const std::byte>{})
...
RefereeTransferResult
UiScheduler::clear_layer(RefereeTransferEndpoint &endpoint,
...
Shape::Shape(UiScheduler &scheduler, Color color, std::uint8_t layer,
             std::uint16_t width)
    : scheduler_(scheduler), color_(color), layer_(layer), width_(width) {
...
Integer::Integer(UiScheduler &scheduler, Color color, std::uint16_t font_size,
                 std::uint16_t width, std::uint16_t x, std::uint16_t y,
                 std::int32_t value, bool visible)
    : Shape(scheduler, color, 0, width), font_size_(font_size), value_(value) {
  set_xy(x, y);
  set_visible(visible);
}
