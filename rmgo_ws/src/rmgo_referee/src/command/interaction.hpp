...
inline void write_u16_le(std::span<std::byte> buffer, std::size_t &written,
                         std::uint16_t value) noexcept {
  buffer[written++] = static_cast<std::byte>(value & 0xFFU);
  buffer[written++] = static_cast<std::byte>((value >> 8U) & 0xFFU);
}

inline void write_u32_le(std::span<std::byte> buffer, std::size_t &written,
...
inline void write_float_le(std::span<std::byte> buffer, std::size_t &written,
...
inline std::optional<InteractiveHeader> make_client_interactive_header(
  ...
  return make_client_interactive_header(endpoint.self_robot_id(),
                                        data_command_id);
}
