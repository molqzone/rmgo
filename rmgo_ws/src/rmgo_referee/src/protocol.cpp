...
constexpr std::array game_robot_hp_fields{
    Field::robots_hp_red_1,       Field::robots_hp_red_2,    Field::robots_hp_red_3,
    Field::robots_hp_red_4,       Field::robots_hp_red_5,    Field::robots_hp_red_7,
    Field::robots_hp_red_outpost, Field::robots_hp_red_base, Field::robots_hp_blue_1,
    Field::robots_hp_blue_2,      Field::robots_hp_blue_3,   Field::robots_hp_blue_4,
    Field::robots_hp_blue_5,      Field::robots_hp_blue_7,   Field::robots_hp_blue_outpost,
    Field::robots_hp_blue_base,
};
...
constexpr std::array radar_mark_fields{
    Field::radar_mark_hero,       Field::radar_mark_engineer,   Field::radar_mark_infantry_3,
    Field::radar_mark_infantry_4, Field::radar_mark_infantry_5, Field::radar_mark_sentry,
};
...
bool apply_frame_to_status(const RefereeFrame& frame, RefereeStatusSink& status) noexcept {
    ...
    case CommandId::game_robot_hp:
        if (data.size() >= game_robot_hp_fields.size() * sizeof(std::uint16_t)) {
            for (std::size_t index = 0; index < game_robot_hp_fields.size(); ++index) {
                status.set(
                    game_robot_hp_fields[index], static_cast<double>(read_u16(data, index * 2)));
            }
        }
        return true;
    case CommandId::event_data:
        if (data.size() >= 4) {
            ...
            status.set(
                Field::event_ally_small_energy_activation_status,
                static_cast<double>((event_data >> 3U) & 0x03U));
            status.set(
                Field::event_ally_big_energy_activation_status,
                static_cast<double>((event_data >> 5U) & 0x03U));
            status.set(
                Field::event_ally_fortress_occupation_status,
    ...
    case CommandId::dart_status:
        if (data.size() >= 3) {
            ...
            status.set(Field::dart_remaining_time, static_cast<double>(read_u8(data, 0)));
            status.set(Field::dart_latest_hit_target, static_cast<double>(dart_info & 0x03U));
            status.set(Field::dart_hit_count, static_cast<double>((dart_info >> 3U) & 0x07U));
            status.set(Field::dart_selected_target, static_cast<double>((dart_info >> 6U) & 0x03U));
        }
        return true;
    case CommandId::robot_status:
        if (data.size() >= 12) {
            ...
            status.set(Field::shooter_heat_limit, static_cast<double>(read_u16(data, 8)));
    ...
    case CommandId::projectile_allowance:
        if (data.size() >= 6) {
            status.set(Field::shooter_bullet_allowance, static_cast<double>(read_u16(data, 0)));
            status.set(
                Field::shooter_42mm_bullet_allowance, static_cast<double>(read_u16(data, 2)));
            status.set(Field::remaining_gold_coin, static_cast<double>(read_u16(data, 4)));
            if (data.size() >= 8) {
                status.set(
                    Field::shooter_fortress_17mm_bullet_allowance,
    ...
    case CommandId::game_robot_position:
        if (data.size() >= ally_robot_position_fields.size() * sizeof(float)) {
            for (std::size_t index = 0; index < ally_robot_position_fields.size(); ++index) {
                status.set(
                    ally_robot_position_fields[index],
    ...
    case CommandId::radar_mark_progress:
        if (data.size() >= radar_mark_fields.size()) {
            for (std::size_t index = 0; index < radar_mark_fields.size(); ++index) {
                status.set(radar_mark_fields[index], static_cast<double>(read_u8(data, index)));
            }
        }
        return true;
    case CommandId::radar_info:
        if (!data.empty()) {
            ...
            status.set(Field::radar_double_effect_chance, static_cast<double>(radar_info & 0x03U));
            status.set(
                Field::radar_double_effect_active, static_cast<double>((radar_info >> 2U) & 0x01U));
        }
        return true;
    case CommandId::sentry_info:
        if (data.size() >= 6) {
            ...
            status.set(
                Field::sentry_exchanged_bullet_allowance,
                static_cast<double>(sentry_info & 0x07FFU));
            status.set(
                Field::sentry_remote_bullet_exchange_count,
                static_cast<double>((sentry_info >> 11U) & 0x0FU));
            status.set(
                Field::sentry_can_confirm_free_revive,
                static_cast<double>((sentry_info >> 19U) & 0x01U));
            status.set(
                Field::sentry_can_exchange_instant_revive,
                static_cast<double>((sentry_info >> 20U) & 0x01U));
            status.set(
                Field::sentry_instant_revive_cost,
                static_cast<double>((sentry_info >> 21U) & 0x03FFU));
            status.set(
                Field::sentry_exchangeable_bullet_allowance,
                static_cast<double>((sentry_info_2 >> 1U) & 0x07FFU));
            status.set(Field::sentry_mode, static_cast<double>((sentry_info_2 >> 12U) & 0x03U));
            status.set(
                Field::sentry_energy_mechanism_activatable,
    ...
    case CommandId::map_command:
        if (data.size() >= 12) {
            ...
            if (target_x != previous_x || target_y != previous_y || keyboard != previous_keyboard
                ...
                status.set(
                    Field::map_command_sequence, status.get(Field::map_command_sequence) + 1.0);
            }
        }
        return true;
    case CommandId::radar_map_robot_data:
        if (data.size() >= opponent_map_robot_position_fields.size() * sizeof(std::uint16_t)) {
            constexpr double centimeter_to_meter = 0.01;
            for (std::size_t index = 0; index < opponent_map_robot_position_fields.size();
                 ++index) {
                status.set(
                    opponent_map_robot_position_fields[index],
                    static_cast<double>(read_u16(data, index * 2)) * centimeter_to_meter);
            }
        }
        return true;
