...
std::chrono::milliseconds frame_budget_period(std::size_t frame_size,
                                              double bytes_per_second) {
  return std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::duration<double>{static_cast<double>(frame_size) /
                                    bytes_per_second});
}
...
Ui::update(RefereeTransferEndpoint &endpoint,
    ...
    if (result == RefereeTransferResult::Accepted) {
      ...
      next_interaction_send_ =
          now + std::max(interaction_period_,
                         frame_budget_period(frame_size,
                                             serial_budget_bytes_per_second_));
    }
    return result;
  }
  ...
  while (auto *shape = cfs_scheduler_->next(
             std::nullopt,
             std::span<Shape *const>{ignored}.first(ignored_count))) {
  ...
  const auto [draw_command, target_count] =
      frame_kind == FrameKind::text
          ? std::pair{InteractiveDataCommandId::client_draw_text,
                      std::size_t{1}}
...
void Ui::register_shape(Shape &shape) {
...
void Ui::run_queue_insert(Shape &shape) {
  cfs_scheduler_->insert(shape);
}

void Ui::run_queue_erase(Shape &shape) noexcept {
  cfs_scheduler_->erase(shape);
}

void Ui::unmark_modified(Shape &shape) noexcept {
  run_queue_erase(shape);
}
