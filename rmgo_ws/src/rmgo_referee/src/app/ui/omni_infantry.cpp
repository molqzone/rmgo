...
std::string value_pair(std::string_view prefix, double value, double limit,
...
std::string value_suffix(std::string_view prefix, double value,
                         std::string_view suffix) {
  std::string content{prefix};
  content.push_back(' ');
  content += std::to_string(round_i32(value));
...
template <typename ShapeArray>
void set_shapes_layer_priority(const ShapeArray &shapes, std::uint8_t layer,
...
public:
  explicit FixedHudWidget(Ui &interaction_ui)
      : center_left_(interaction_ui, Color::cyan, 2, x_center - 96, y_center,
                     x_center - 24, y_center, false),
        center_right_(interaction_ui, Color::cyan, 2, x_center + 24, y_center,
                      x_center + 96, y_center, false),
        center_up_(interaction_ui, Color::cyan, 2, x_center, y_center - 72,
                   x_center, y_center - 22, false),
        center_down_(interaction_ui, Color::cyan, 2, x_center, y_center + 22,
                     x_center, y_center + 72, false),
        center_ring_(interaction_ui, Color::white, 2, x_center, y_center, 11,
                     false),
        range_left_(interaction_ui, Color::white, 2, x_center - 160,
                    y_center + 92, x_center - 54, y_center + 92, false),
        range_right_(interaction_ui, Color::white, 2, x_center + 54,
                     y_center + 92, x_center + 160, y_center + 92, false),
        lane_left_(interaction_ui, Color::cyan, 3, 610, 1008, 884, 620, false),
        lane_right_(interaction_ui, Color::cyan, 3, 1310, 1008, 1036, 620,
                    false),
        lane_left_inner_(interaction_ui, Color::white, 1, 760, 980, 910, 690,
                         false),
        lane_right_inner_(interaction_ui, Color::white, 1, 1160, 980, 1010, 690,
                          false),
        horizon_(interaction_ui, Color::white, 1, x_center - 210,
                 y_center + 180, x_center + 210, y_center + 180, false),
        rotation_(interaction_ui, Color::yellow, 4, x_center, y_center,
                  x_center, y_center - 80, false) {
  ...
  void update(const RefereeUiState &state) {
    ...
    lane_left_.set_end_xy(screen_coordinate(884.0 - pitch * 80.0),
                          screen_coordinate(lane_top_y));
    lane_right_.set_end_xy(screen_coordinate(1036.0 + pitch * 80.0),
                           screen_coordinate(lane_top_y));
    lane_left_inner_.set_end_xy(screen_coordinate(910.0 - pitch * 45.0),
                                screen_coordinate(lane_inner_top_y));
    lane_right_inner_.set_end_xy(screen_coordinate(1010.0 + pitch * 45.0),
                                 screen_coordinate(lane_inner_top_y));

    const double yaw =
        std::isfinite(state.chassis_yaw) ? state.chassis_yaw : 0.0;
    rotation_.set_end_xy(
        screen_coordinate(static_cast<double>(x_center) + std::sin(yaw) * 90.0),
        screen_coordinate(static_cast<double>(y_center) -
                          std::cos(yaw) * 90.0));
    rotation_.set_color(round_i32(state.chassis_mode) == 0 ? Color::green
                                                           : Color::pink);
  }
...
class ModeWidget {
public:
  explicit ModeWidget(Ui &interaction_ui)
      : chassis_(interaction_ui, Color::green, 18, 2, 120, 790, "CHS raw",
                 false),
        gimbal_(interaction_ui, Color::pink, 18, 2, 120, 830, "GMB off", false),
        shooter_(interaction_ui, Color::white, 18, 2, 120, 870, "SHT stop",
                 false),
        game_(interaction_ui, Color::white, 18, 2, 120, 910, "T 0s", false),
        online_(interaction_ui, Color::green, 18, 2, 120, 950, "REF online",
                false) {
  ...
  void update(const RefereeUiState &state) {
    ...
    gimbal_.set_color(state.remote_gimbal_eject > 0.5 ? Color::orange
                      : gimbal_enabled                ? Color::green
                                                      : Color::pink);
...
private:
  std::array<Shape *, 5> shapes() {
    return {&chassis_, &gimbal_, &shooter_, &game_, &online_};
  }

  static Color chassis_color(std::string_view mode, double output_status,
                             double power_limit_state) {
...
class BarWidget {
public:
  BarWidget(Ui &interaction_ui, std::uint16_t x, std::uint16_t y,
            std::uint16_t length, std::string label)
      : label_(interaction_ui, Color::white, 16, 2, x - 76, y - 7,
        ...
        text_(interaction_ui, Color::white, 16, 2, x + length + 18, y - 7, "",
              false),
        x_(x), y_(y), length_(length) {
  ...
  void update(double current, double limit, std::string_view suffix,
              Color color) {
    const auto progress = ratio(current, limit);
    value_.set_end_xy(static_cast<std::uint16_t>(
                          x_ + static_cast<std::uint16_t>(length_ * progress)),
                      y_);
...
class PowerWidget {
public:
  explicit PowerWidget(Ui &interaction_ui)
...
class HeatWidget {
public:
  explicit HeatWidget(Ui &interaction_ui)
      : heat_(interaction_ui, 610, 180, 600, "HEAT"),
        cooling_(interaction_ui, Color::white, 16, 2, 1228, 211, "COOL 0",
                 false) {
...
public:
  ...
  void update(const RefereeUiState &state) {
    const auto allowance =
        std::max(0, round_i32(state.shooter_bullet_allowance));
...
class TargetWidget {
public:
  explicit TargetWidget(Ui &interaction_ui)
      : target_(interaction_ui, Color::cyan, 18, 2, 120, 710, "TGT target_none",
                false),
        lock_box_(interaction_ui, Color::white, 3, 500, 240, 1420, 840, false),
        distance_(interaction_ui, Color::white, 18, 2, 1450, 270, "DIST 0m",
                  false) {
  ...
  void update(const RefereeUiState &state) {
    const auto name =
        target_name(state.remote_target, state.remote_armor_target);
...
public:
  explicit CoverFlashWidget(Ui &interaction_ui)
      : text_(interaction_ui, Color::green, 25, 2, 830, 700, "cover open!!",
              false) {
    text_.set_layer(1);
...
public:
  explicit FrictionSpeedWidget(Ui &interaction_ui)
...
public:
  explicit OmniInfantryUiProfile(Ui &interaction_ui)
      : fixed_(interaction_ui), modes_(interaction_ui), health_(interaction_ui),
        power_(interaction_ui), heat_(interaction_ui), bullets_(interaction_ui),
        target_(interaction_ui), cover_(interaction_ui),
...
std::unique_ptr<UiProfile> make_ui_profile(std::string_view name,
                                           Ui &interaction_ui) {
