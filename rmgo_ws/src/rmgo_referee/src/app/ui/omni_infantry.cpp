...
public:
    explicit FixedHudWidget(Ui& interaction_ui)
        ...
        , horizon_(
              ...
              y_center + 180, false)
        , rotation_(
              interaction_ui, Color::yellow, 4, x_center, y_center, x_center, y_center - 80,
              false) {
    ...
    void update(const RefereeUiState& state) {
        ...
        lane_left_.set_end_xy(
            screen_coordinate(884.0 - pitch * 80.0), screen_coordinate(lane_top_y));
...
private:
    std::array<Shape*, 13> shapes() {
        return {
            &center_left_,      &center_right_, &center_up_, &center_down_, &center_ring_,
            &range_left_,       &range_right_,  &lane_left_, &lane_right_,  &lane_left_inner_,
            &lane_right_inner_, &horizon_,      &rotation_,
        };
    }
...
public:
    BarWidget(
        Ui& interaction_ui, std::uint16_t x, std::uint16_t y, std::uint16_t length,
        std::string label)
